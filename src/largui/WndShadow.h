...
#pragma warning(pop) 
	...
	// Restore last parent window size, used to determine the updateTimeWidget strategy when parent window is resized
	LPARAM m_WndSize;

	// Set this to true if the shadow should not be updateTimeWidget until next WM_PAINT is received
	bool m_bUpdate;
...
public:
	...
	bool SetHideSingleShadow(ShadowExMode showMode);
	void HideShadow();// 隐藏此对象创建的阴影窗口
	void ShowShadow();// 恢复并显示此对象创建的阴影窗口
